...
| 库 | 用途 | 备注 |
| --- | --- | --- |
| `react-markdown` | 渲染便笺 Markdown 内容 | 配合 `remark-gfm` 支持 GFM (任务列表等) |
| `remark-gfm` | Markdown 扩展语法 (表格/删除线/任务列表/链接) | `react-markdown` 插件 |
...
**需求:** 拖拽便笺可移动; 拖拽未松开时显示蓝色半透明 mask 预览新位置, 松开后提交位置更新。

**UI 表现:** 按住便笺主体 (图钉除外) 拖动 → 原便笺半透明变淡, 目标位置出现蓝色半透明描边矩形 (如 `bg-blue-500/20 border-2 border-blue-500`); 松开 → mask 变为实际位置, 便笺到位, 经服务端 Pusher 广播同步给他人。

**实现计划:**
- 手势: pointerdown 于 NoteCard 主体 → 进入 `dragNote` 状态 (store), 记录起始屏幕坐标与世界坐标差值;
- 拖动中: 只更新 store 中的 `previewPosition` (纯前端), NoteCard 根据视口变换渲染在预览位置并降透明度; `DragPreview` 渲染蓝色 mask (世界坐标);
- 松开: 调用 `PUT /api/notes/:id/position` → 成功后 store 提交新位置 (服务端经 Pusher 广播 patch); 失败回滚到原位置;
- 拖拽期间阻止缩放/平移冲突 (`e.stopPropagation()` + pointer capture)。

### F5 便笺编辑器 (Markdown + 图片 + 快捷状态)

**需求:** 点击便笺进入编辑页; 支持 Markdown 语法 (标题、无序/有序列表、外链、图片等) 渲染与输入; 可插入图片; 提供快捷状态栏: 心情 / 天气 / 疲惫程度 / 进食。

**UI 表现:** 独立路由页 `/b/:boardId/n/:noteId`; 左侧编辑区 (textarea + 格式工具栏), 右侧实时预览 (Markdown 渲染); 工具栏按钮: 加粗/斜体/标题/无序列表/有序列表/链接/图片; 快捷状态栏为卡片式四个分组:
- 心情 (mood): 😊 开心 · 😐 一般 · 😔 低落 · 😡 生气 · 😴 困倦 (lucide 或 emoji);
- 天气 (weather): ☀️ 晴 · 🌤 多云 · 🌧 雨 · 🌨 雪 · ⛈ 雷暴;
- 疲惫 (fatigue): 0~10 滑块 + 数字;
- 进食 (diet): 自由文本输入 ("早餐: 豆浆油条").

**实现计划:**
- 路由 `note.tsx`: loader 校验权限并取 note + board 元数据; 懒加载编辑区组件;
- 渲染: `react-markdown` + `remark-gfm`, 自定义 `components` (链接新标签打开、图片自适应宽度、标题/列表/代码风格化); 预览随输入 debounce 300ms 更新;
- 工具栏: 插入语法 (如 `**粗体**`), 文本选中时包裹;
- 图片插入: 文件选择 → 压缩 (可选, canvas) → `POST /api/images` → 返回 URL 插入 `![图片](url)`; 粘贴图片 (clipboard) 同样支持;
